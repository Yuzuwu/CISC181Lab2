...
public class HandPoker extends Hand implements Comparable {
	...
	private boolean isRoyalFlush() {
		...
		if (sCount == 5) {
			for(Card c:this.getCards()) {
				if (c.geteRank() == eRank.ACE || c.geteRank() == eRank.KING || c.geteRank() == eRank.QUEEN|| c.geteRank() == eRank.JACK || c.geteRank() == eRank.TEN){
					rCount++;
				}
			}
			if (rCount == 5) {
		...
		// TODO - (finished)Complete implementation for this method.
	...
	private boolean isFourOfAKind() {
		...
		// TODO - (finished) Complete implementation for this method.  done
	...
	private boolean isStraight() {
		...
		if ((GetCRCSize()==eRowCount.FIVE.getiRowCountItems())) {
			if (this.getCards().get(0).geteRank()==eRank.ACE) {
				if ((this.getCards().get(1).geteRank()==eRank.KING)||(this.getCards().get(1).geteRank()==eRank.FIVE)){
					for (int i =2; i<5;i++) {
						if(this.getCards().get(i).geteRank().getiRankNbr()-this.getCards().get(i-1).geteRank().getiRankNbr()==-1)
							bisStraight = true;
						if (this.getCards().get(1).geteRank()==eRank.FIVE) {
							HandScorePoker HSP = (HandScorePoker) this.getHS();
